...
namespace WiseM.Client
{
    public partial class Box_Palletizing : SkinForm
    {
        ...
        private List<SerialPort> _autoScannerList = new List<SerialPort>();
        ...
        public Box_Palletizing(string type, string date)
        {
            InitializeComponent();
            ...
            try
            {
                ...
                StringBuilder tempStrComPortName1 = new StringBuilder();
                StringBuilder tempStrComPortName2 = new StringBuilder();
                StringBuilder tempStrComPortName3 = new StringBuilder();
                StringBuilder tempStrComPortName4 = new StringBuilder();
        ...
        private void Box_Palletizing_Load(object sender, EventArgs e)
        {
            try
            {
                ...
                this.tmrIgnoreDataFromSerialPort2.Elapsed += TmrIgnoreDataFromSerialPort2_Elapsed;
                this.tmrIgnoreDataFromSerialPort3.Elapsed += TmrIgnoreDataFromSerialPort3_Elapsed;
        ...
        private void Init()
        {
            try
            {
                ...
                try
                {
                    ...
                    string ini_Q;
                    ini_Q = $@"
                    SELECT COALESCE(B.Count, 0) + COALESCE(OH.OutQty, 0) Cnt FROM WorkOrder WO
                    LEFT OUTER JOIN (SELECT B.WorkOrder, COUNT(( B.PcbBcd )) AS Count FROM BoxTemp B WHERE 1 = 1 AND B.PalletBcd IS NULL GROUP BY B.WorkOrder) B ON WO.WorkOrder = B.WorkOrder
                    LEFT OUTER JOIN (SELECT SUM(OH.OutQty) AS OutQty, OH.WorkOrder FROM OutputHist AS OH GROUP BY OH.WorkOrder) OH ON WO.WorkOrder = OH.WorkOrder
                    ...
                    lbl_item.Text = _Material + " / " + dtQ.Rows[0]["Spec"].ToString();
                    _orderQty = dtQ.Rows[0]["OrderQty"].ToString();
                    ...
                    materialText = dtQ.Rows[0]["Text"].ToString();
                    spec = dtQ.Rows[0]["Spec"].ToString();

                    lbl_Qty.Text = string.Format("{0} / {1}", count_pcb, _orderQty);
                    lbl_BoxQty.Text = string.Format("{0} / {1}", count_noBoxingPcb, _boxQty);
                    lbl_PalletQty.Text = string.Format("{0} / {1}", count_box, _palletQty);

                    if (dt_ini.Tables[4].Rows.Count <= 0) lbl_BoxBarcode.Text = "-";
                    else lbl_BoxBarcode.Text = dt_ini.Tables[4].Rows[0]["BoxBcd"].ToString();

                    if (dt_ini.Tables[5].Rows.Count <= 0) lbl_PalletBarcode.Text = "-";
                    else lbl_PalletBarcode.Text = dt_ini.Tables[5].Rows[0]["PalletBcd"].ToString();
                }
                catch (Exception ex)
                {
                ...
                {
                    this.lbl_prev2.Visible = false;
                    this.btn_Reset.Visible = false;
                    this.tableLayoutPanel12.SetColumnSpan(this.tableLayoutPanel14, 2);
                    this.tableLayoutPanel1.SetColumnSpan(this.lbl_prev1, 2);
                    lbl_prev1.Text = string.Empty;

                    this.lbl_Current2.Visible = false;
                    this.tableLayoutPanel1.SetColumnSpan(this.lbl_Current1, 2);
                    lbl_Current1.Text = string.Empty;
                }
                else
                {
                    this.lbl_prev2.Visible = true;
                    this.tableLayoutPanel16.SetColumnSpan(this.lbl_prev1, 1);
                    lbl_prev1.Text = string.Empty;
                    lbl_prev2.Text = string.Empty;

                    this.lbl_Current2.Visible = true;
                    this.tableLayoutPanel15.SetColumnSpan(this.lbl_Current1, 1);
                    lbl_Current1.Text = string.Empty;
                ...
                if (lbl_type.Text == "SINGLE")
                {
                    this.serialPort2.PortName = this.singlePort;
                    this.serialPort2.BaudRate = 115200;
                    this.serialPort2.DataBits = 8;
                    this.serialPort2.StopBits = StopBits.One;
                    this.serialPort2.NewLine = "\r\n";
                    this.serialPort2.Open();
                    this.serialPort2.DataReceived += SerialPort2_DataReceived;
                }
                else
                {
                    this.serialPort2.PortName = this.leftPort;
                    this.serialPort2.BaudRate = 115200;
                    this.serialPort2.DataBits = 8;
                    this.serialPort2.StopBits = StopBits.One;
                    this.serialPort2.NewLine = "\r\n";
                    ...
                    this.serialPort3.PortName = this.rightPort;
                    this.serialPort3.BaudRate = 115200;
                    this.serialPort3.DataBits = 8;
                    this.serialPort3.StopBits = StopBits.One;
                    this.serialPort3.NewLine = "\r\n";
                    this.serialPort3.Open();
                    this.serialPort3.DataReceived += SerialPort3_DataReceived;
                }
        ...
        private void SerialPort2_DataReceived(object sender, SerialDataReceivedEventArgs e)
        {
            ...
            if (this.boolIgnoreDataFromSerialPort2 == true)
            {
            ...
            this.tmrIgnoreDataFromSerialPort2.Start();
        ...
        private void SerialPort3_DataReceived(object sender, SerialDataReceivedEventArgs e)
        {
            ...
            if (this.boolIgnoreDataFromSerialPort3 == true)
            {
                SetLogMessage("Scanner scan signal received again");
                return;
            }
            ...
            this.tmrIgnoreDataFromSerialPort3.Start();
        ...
        private void ProcessingSINGLE(string scanData)
        {
            ...
            try
            {
                ...
                else if (ds1.Tables[ds1.Tables.Count - 1].Rows[0]["RC"].ToString() != "0")
                {
                ...
                if (ds1.Tables[0].Rows[0]["RTN_TXT"].ToString() == "OK")    // Verify OK
                {
                    ...
                    lbl_BoxQty.Text = $@"{ds_pcb.Tables[1].Rows[0]["PcbCnt"].ToString()} / {_boxQty}";
        ...
        private void Processing1stDOUBLE(string scanData)
        {
            ...
            try
            {
                ...
                if (ds1 == null || ds1.Tables.Count == 0)
                {
                    ...
                    this.lbl_Current1.ForeColor = Color.Red;
                    this.lbl_Error.Visible = true;
                    this.lbl_Error.Text = strErr + " / " + this.str2ndErrMsg;

                    if (this.int2ndLabelFlag == 0)      // 2nd에서 처리
                    {
                        this.int1stLabelFlag = 2;
                        this.str1stErrMsg = strErr;
                        return;
                    }
                    else if (this.int2ndLabelFlag == 1)
                    {
                        this.AllFlagReset();
                ...
                else if (ds1.Tables[0].Rows[0]["RTN_TXT"].ToString() == "OK")    // Verify OK
                {
                    ...
                    else if (this.int2ndLabelFlag == 1)          // OK 실적처리
                    {
                        ////////////////this.AllFlagReset();

                        if (scanData == this.str2ndLabel)
                        {
        ...
        private void Processing2ndDOUBLE(string scanData)
        {
            ...
            try
            {
                ...
                if (ds1 == null || ds1.Tables.Count == 0)
                {
                    ...
                    this.lbl_Current2.ForeColor = Color.Red;
                    this.lbl_Error.Visible = true;
                    this.lbl_Error.Text = this.str1stErrMsg + " / " + strErr;

                    if (this.int1stLabelFlag == 0)      // 1st에서 처리
                    {
                        this.int2ndLabelFlag = 2;
                        this.str2ndErrMsg = strErr;
                        return;
                    }
                    else       // 1  or 2
                    {
                ...
                if (ds1.Tables[0].Rows[0]["RTN_TXT"].ToString() == "OK") // Verify OK
                {
                    switch (this.int1stLabelFlag)
                    {
                        // 1st에서 처리
                        case 0:
                            this.str2ndLabel = scanData;
                            this.int2ndLabelFlag = 1;
                            break;
                        // OK 실적처리
                        ////////////////this.AllFlagReset();
                        case 1 when scanData == this.str1stLabel:
                        {
                            string strErr = "The 1st barcode and the 2nd barcode are the same.";
                            ...
                            SetLogMessage($"[{str1stLabel}], [{scanData}] {this.str1stErrMsg} / {strErr}");
                        ...
                        default:
                            lbl_Current2.ForeColor = Color.Red;
                            this.lbl_Current2.Text = "N/A";
                            lbl_Error.Visible = true;
                            lbl_Error.Text = this.str1stErrMsg + " / " + str2ndErrMsg;
                ...
                else
                {
                    ...
                    else if (this.int1stLabelFlag == 1)
                    {
        ...
        {
            this.int1stLabelFlag = 0;
            this.int2ndLabelFlag = 0;
            this.str1stLabel = "";
            this.str2ndLabel = "";
            this.str1stErrMsg = "";
            this.str2ndErrMsg = "";
        ...
        private void WriteDigitalOut(int intAdrs, bool bVal)
        {
            try
            {
                ...
                for (i = 0; i < 15; i++)
                {
                    if (this.boolDioAvailable) break;
                    Thread.Sleep(100);
                ...
                byte[] bCmdPresetSingleRegister = { this.bSlaveId, 0x06, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00 };
                ...
                this.serialPort1.Write(bCmdPresetSingleRegister, 0, bCmdPresetSingleRegister.Length);

                if (intAdrs == 1 && bVal) this.tmrDio1.Start();
                else if (intAdrs == 2 && bVal) this.tmrDio2.Start();

                this.boolDioAvailable = false;
                this.tmrDioReply.Start();
            }
            catch (Exception ex)
            {
                this.InsertIntoSysLog(ex.Message);
        ...
        private void VerifyPrint_Box()
        {
            try
            {
                ...
                if (int.Parse(currentQty) >= int.Parse(_boxQty))
                {
                ...
                if (_palletQty == currentQty)
                {
                    ...
                    try
                    {
                        string Query_pallet = $@"
                                            ...
                                            SELECT ROW_NUMBER() OVER (ORDER BY BT.Updated) AS Cnt, BT.BoxBcd, BT.Updated
                                            FROM (SELECT BT.BoxBcd, MAX(BT.Updated) AS Updated FROM boxtemp BT WHERE and WorkCenter = '{_Workcenter}' AND PalletBcd IS NULL AND BoxBcd IS NOT NULL
                                            GROUP BY BT.BoxBcd) AS BT ORDER BY BT.Updated
                                                ";
        ...
        private bool Boxing(string barcode, string qty)
        {
            try
            {
                ...
                string updateQuery2 = $@"
                                    UPDATE KeyRelation
                                    SET Box_WorkOrder = '{_Workorder}', Box_Material = '{_Material}', Box_Line = '{_Workcenter}', Boxed = CONVERT(VARCHAR, GETDATE(), 20), Boxbcd = '{barcode}', Updated = GETDATE(), UpdateUser = '{WiseApp.Id}'
                                    FROM (SELECT PcbBcd from BoxTemp where BoxBcd = '{barcode}') BT
        ...
        private void Palletizing(string barcode, string qty)
        {
            try
            {
                ...
                insertQuery = $@"
                            ...
                            FROM BoxTemp WHERE WorkOrder = '{_Workorder}' AND WorkCenter = '{_Workcenter}'
                ...
                string Q = $@"SELECT count(*) FROM BoxTemp where PalletBcd = '{barcode}'";
        ...
        public void Good(int count)
        {
            try
            {
                ...
                r.UserColumns = new System.Collections.Generic.SortedList<string, object>();
                r.From = WiseApp.Id.ToString();
                ...
                r.Count = count;
                r.Cavity = 1;
                WiseM.JobControl.Good.Insert(r);
            }
            catch (Exception ex)
            {
                this.InsertIntoSysLog(ex.Message);
            ...
            //WiseM.Client.UserControls.WbtRefresh wbtRefresh = new WiseM.Client.UserControls.WbtRefresh();
            //wbtRefresh.RWbt();
        }
        private void Print(string ptype, string currentqty)
        {
            ...
            else
            {
                try
                {
                    ...
                    else if (ptype.Equals("Palletizing"))
                    {
                        ...
                        _clsBarcode.Data.SetText("QTY", qty.ToString() + " EA");
        ...
        private void SetLogMessage(string message)
        {
            ...
            FileInfo fileInfo = new FileInfo(path + $@"\{DateTime.Now.ToString("yyyy-MM-dd")}.txt");

            if (!fileInfo.Exists)
            {
                System.IO.File.AppendAllText(path + $@"\{DateTime.Now.ToString("yyyy-MM-dd")}.txt", logMessage, Encoding.Default);
            }
        ...
        private void Box_Palletizing_FormClosing(object sender, FormClosingEventArgs e)
        {
            try
            {
                ...
                this.tmrIgnoreDataFromSerialPort2.Dispose();
                this.tmrIgnoreDataFromSerialPort3.Dispose();
        ...
        private void Process_threads_Clear()
        {
            ...
            else
            {
                ...
                MessageBoxButtons.YesNo, MessageBoxIcon.Question) == DialogResult.Yes)
                {
                    ...
                    lbl_Qty.Text = string.Format("{0} / {1}", count_pcb, _orderQty);
                    lbl_BoxQty.Text = string.Format("{0} / {1}", count_nonBoxingPcb, _boxQty);
                }
        ...
        private void btn_Reset_Click(object sender, EventArgs e)
        {
            ...
            {
                if (lbl_Current1.ForeColor != Color.Red && lbl_Current2.ForeColor != Color.Red) return;
            ...
            else
            {
                try
                {
                    ...
                    MessageBoxButtons.YesNo, MessageBoxIcon.Question) == DialogResult.Yes)
                    {
                        ...
                        string productionDate = dt.Rows[0]["ProductionDate"].ToString();

                        string tempqty = $"{strQty:0#}";

                        string strSql = string.Format("SELECT BcdData FROM BcdLblFmtr WHERE BcdName='Label_Box'");
                        DataTable dtMain = DbAccess.Default.GetDataTable(strSql);
                ...
                try
                {
                    ...
                    MessageBoxButtons.YesNo, MessageBoxIcon.Question) == DialogResult.Yes)
                    {
                        ...
                        string insert_Q = $@"
                                ...
                                (LG_PartNo, Model, Qty, PalletBcd, Mfg_ymd, Mfg_Line, ProductionDate, ProductionLine,
                                Material, SerialNo, Reprint, Updated, Updater)
                                select top 1 LG_PartNo, Model, Qty, PalletBcd, Mfg_ymd, Mfg_Line, ProductionDate, ProductionLine,
                                Material, SerialNo, Reprint + 1, Updated, Updater from PalletbcdPrintHist with(nolock)
                                WHERE PalletBcd = '{lbl_PalletBarcode.Text}' ORDER BY SerialNo desc
                                ";
                        DbAccess.Default.ExecuteQuery(insert_Q);
                    }
