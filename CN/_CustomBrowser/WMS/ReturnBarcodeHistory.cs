...
namespace WiseM.Browser.WMS
{
    public partial class ReturnBarcodeHistory : Form
    {
        ...
        private void button_Save_Click(object sender, EventArgs e)
        {
            ...
            if (System.Windows.Forms.MessageBox.Show("Save?", "Confirm", MessageBoxButtons.YesNo) != DialogResult.Yes)
            {
                return;
            }
            try
            ...
            {
                if (dataGridView_RawMaterial.Rows[i].Cells[1].Value.ToString() != textBox_Scan_Material.Text) continue;
        ...
        private void button_Add_Click(object sender, EventArgs e)
        {
            ...
            if (string.IsNullOrEmpty(comboBox_From.Text))
            {
            ...
            if (dataGridView_ScanList.Rows.Count > 0)
            {
                ...
                {
                    if (textBox_Scan_Barcode.Text == dataGridView_ScanList.Rows[i].Cells["Rm_BarCode"].Value.ToString())
                    {
                        ClearBarcodeInfo();
        ...
        private void Init()
        {
            ...
            comboBox_location_group.DataSource = dataSet.Tables[0];

            comboBox_From.DisplayMember = "Text";
            comboBox_From.ValueMember = "SL_CD";
            dataSet.Tables[1].Rows.Add("", "");
...
";
        ...
        private void ScanRawMaterial()
        {
            ...
            if (dataGridView_ScanList.Rows.Count < 1) return;
            if (System.Windows.Forms.MessageBox.Show("Add?", "Confirm", MessageBoxButtons.YesNo) != DialogResult.Yes) return;             
