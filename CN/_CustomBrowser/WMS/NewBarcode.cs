...
namespace WiseM.Browser.WMS
{
    public partial class NewBarcode : Form
    {
            ...
            switch (rawMaterialPrintType)
            {
                ...
                case CustomEnum.RawMaterialPrintType.Change:
                    ...
                    afterQuery.AppendLine
                        ...
                        --ERP 전송
                        INSERT
                          INTO MES_IF_CN.dbo.MTE_INV_TRANS ( IF_TIME
                                                           , I_PROC_STEP
                                                           , I_APPLY_STATUS
                                                           , DOCUMENT_DT
                                                           , MOV_TYPE
                ...
                case CustomEnum.RawMaterialPrintType.Reprint:
                    ...
                    {
                        MessageBox.Show("条形码输出历史不存在。(No print out history.)", "无输出历史。(No print out history.)", MessageBoxIcon.Warning);
                        return;
                    }
                ...
                case CustomEnum.RawMaterialPrintType.Remain:
                    ...
                    foreach (DataGridViewRow dataGridViewRow in dataGridView_BoxList.Rows)
                    {
                        ...
                        for (var i = 0; i < boxQty; i++)
                        {
                            ...
                            cBarcode.Data.SetText("LEVEL", string.IsNullOrEmpty(moistureSensitivityLevel) ? $"" : $"MSL:{moistureSensitivityLevel}");
                            WritePrintHist
                                (barcode, material, supplier, date, qty, seq.ToString("00#"));
                            //cBarcode.Data.Clear();
                ...
                case CustomEnum.RawMaterialPrintType.Split:
                    ...
                    {
                        MessageBox.Show("是未入库的条形码。(Barcode not found.)", "未入库(Barcode not found.)", MessageBoxIcon.Warning);
                        return;
                    }
        ...
        {
            if (MessageBox.Show("你想把它打印出来吗？(Do you want to print it out?)", "确认(Confirm)", MessageBoxButtons.YesNo, MessageBoxIcon.Question, MessageBoxDefaultButton.Button1) != DialogResult.Yes) return;
        ...
        private void button_Add_Click(object sender, EventArgs e)
        {
            ...
            {
                MessageBox.Show("请输入品种。(Please enter the material.)", "警告(Warning)", MessageBoxButtons.OK, MessageBoxIcon.Warning);
                return;
            }

            if (string.IsNullOrEmpty(textBox_Supplier.Text))
            {
                MessageBox.Show("请输入供应商。(Please select the supplier.)", "警告(Warning)", MessageBoxButtons.OK, MessageBoxIcon.Warning);
                return;
            }
            ...
            if (_currentPrintType == CustomEnum.RawMaterialPrintType.Split)
            {
                ...
                {
                    //수량이 초과되었습니다.
                    MessageBox.Show("数量超标了。(Quantity exceeded.)", "超过数量(Quantity exceeded.)", MessageBoxIcon.Warning);
                    return;
                }
            }
        ...
        private void textBox_Material_KeyDown(object sender, KeyEventArgs e)
        {
                    ...
                    RM.RawMaterial
                     ...
                     , CASE
                           ...
                           WHEN RM.Type = 'Reel'
                               ...
                               ELSE 'Box'
                       END                         AS Type
                     , COALESCE(RMBS.Supplier, '') AS Supplier
            ...
            if (dataRow is null)
            {
                ...
                textBox_Supplier.Text = "";
                textBox_SupplierName.Text = "";
                MessageBox.Show("未找到品种信息。(Material information not found.)", "警告(Warning)", MessageBoxButtons.OK, MessageBoxIcon.Warning);
            }
            else
            {
        ...
        private void textBox_RePrint_KeyDown(object sender, KeyEventArgs e)
        {
            ...
            {
                MessageBox.Show("仓库里不存在的条形码。(Nonexistent barcode.)", "不存在(Nonexistent)", MessageBoxIcon.Warning);
                return;
            }
        ...
        private bool AddSplitScanList(string barcode)
        {
            try
            {
                ...
                {
                    MessageBox.Show("已扫描资材。(Recently scanned material.)", "不存在(Recently scanned)", MessageBoxIcon.Warning);
                    return false;
                }
                ...
                {
                    MessageBox.Show("仓库里存在的条形码。(existent barcode.)", "不存在(existent)", MessageBoxIcon.Warning);
                    return false;
                }
