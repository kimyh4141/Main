            ...
            switch (e.Program.ToLower())
            {
                ...
                case "jig004":
                    switch (e.Link.ToLower())
                    {
                        ...
                        case "delete" when WiseApp.Id == "gadi74" || WiseApp.Id == "shlee" || WiseApp.Id == "hykim" || WiseApp.Id == "ybkim":
                        {
                ...
                case "jig103":
                    ...
                    if (e.Link.ToLower() == "start")
                    {
                        ...
                        for (int i = 0; i < selectCells.Length; i++)
                        {
                            string JigCode = string.Empty;

                            DataTable dt = DbAccess.Default.GetDataTable($"Select * From JigMaintHist where Jig = '{e.DataGridView.Rows[selectCells[i]].Cells["Jig"].Value}' and ConfirmDate is null ");
                            if (dt.Rows.Count > 0) continue;
                            JigCode = e.DataGridView.Rows[selectCells[i]].Cells["Jig"].Value.ToString();
                                ...
                                ))
                        {
                            ...
                            try
                            {
                                ...
                                foreach (DataTable dataTable in dataSet.Tables)
                                {
                                    ...
                                    {
                                        System.Windows.Forms.MessageBox.Show("查询失败(Query failed.)");
                                        return;
                                    }
                            ...
                            // Message 성공
                            System.Windows.Forms.MessageBox.Show($"已保存\r\nSave complete");
                        }
                    }
                    ...
                    if (e.Link.ToLower() == "download")
                    {
                                ...
                                ))
                        {
                            ...
                            foreach (DataGridViewRow dataGridViewSelectedRow in dataGridViewSelectedRowCollection)
                            {
                                ...
                                SELECT RIGHT([Source], CHARINDEX('\', REVERSE([Source]))-1) AS Filename
                                 , E_RawData
                                  FROM Y2sCn1iData.dbo.First_Func_Test
                                 WHERE PcbId = '{pcbId}' AND Tested = '{tested}'
                                ;
                               "
                                    );
                    ...
                    if (e.Link.ToLower() == "download")
                    {
                                ...
                                ))
                        {
                            ...
                            foreach (DataGridViewRow dataGridViewSelectedRow in dataGridViewSelectedRowCollection)
                            {
                                ...
                                SELECT RIGHT([Source], CHARINDEX('\', REVERSE([Source]))-1) AS Filename
                                 , E_RawData
                                  FROM Y2sCn1iData.dbo.Second_Func_Test
                                 WHERE PcbId = '{pcbId}' AND Tested = '{tested}'
                                ;
                               "
                                    );
                ...
                case "wms001":
                    ...
                    {
                        case "BarCodePrint":
                            var newBarcode = new WMS.NewBarcode();
                            newBarcode.ShowDialog();
                            break;
                        case "BarCodeReturn":
                            var returnBarcodeHistory = new WMS.ReturnBarcodeHistory();
                ...
                case "wms002":
                    switch (e.Link.ToLower())
                    {
                        case "print_pcb":
                        {
                            ...
                            pp.Text = "PCB条码标签打印程序 (PCB Barcode Label Printing Program)";
                            pp.ShowDialog();
                            e.AfterRefresh = WeRefreshPanel.Current;
                            e.AfterRefresh = WeRefreshPanel.Parent;
                        ...
                        case "reprint":
                        {
                            ...
                            rpp.Text = "PCB 条码补发计划 (PCB Barcode Reprint Program)";
                            rpp.ShowDialog();
                            e.AfterRefresh = WeRefreshPanel.Current;
                            break;
                        }
