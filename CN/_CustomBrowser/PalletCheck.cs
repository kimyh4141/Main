...
"
            ...
            try
            {
                ...
                if (dataRow == null)
                {
                    System.Windows.Forms.MessageBox.Show($@"无标签信息。(Label information does not exist.)", "警告(Warning)", MessageBoxButtons.OK, MessageBoxIcon.Warning);
                    return false;
                }
        ...
        private bool ScanBox(string barcode)
        {
            try
            {
                if (0 < DbAccess.Default.IsExist("Stock", $"PalletBcd = '{textBox_PalletScanBarcode.Text}' AND BoxBcd = '{barcode}'"))
                {
                    ...
                    {
                        System.Windows.Forms.MessageBox.Show($"已存在。(Already included in Hist.)", "警告(Warning)", MessageBoxButtons.OK, MessageBoxIcon.Warning);
                        return false;
                    }
...
"
        ...
        private void button_Save_Click(object sender, EventArgs e)
        {
            ...
            {
                System.Windows.Forms.MessageBox.Show($@"无更改内容。(Nothing has changed.)", "警告(Warning)", MessageBoxButtons.OK, MessageBoxIcon.Warning);
                return;
            }

            if (DialogResult.Yes != System.Windows.Forms.MessageBox.Show("你确定吗？(Are you sure?)", "问题(Question)", MessageBoxButtons.YesNo, MessageBoxIcon.Question)) return;
            if (Save())
            {
                //저장완료 메시지
        ...
        {
            if (DialogResult.Yes == System.Windows.Forms.MessageBox.Show("你确定吗？(Are you sure?)", "问题(Question)", MessageBoxButtons.YesNo, MessageBoxIcon.Asterisk))
            {
                Clear();
            }
        }
