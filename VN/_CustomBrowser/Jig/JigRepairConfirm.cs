...
{
    public partial class JigRepairConfirm : SkinForm
    {
        //private CustomPanelLinkEventArgs e = null;
        private string JigMaintHist = string.Empty;
        private string JigCode = string.Empty;
        private string date = string.Empty;
   
        public JigRepairConfirm(string JigCode1, string JigMainthist1)
        {
            JigCode = JigCode1;
            JigMaintHist = JigMainthist1;
            InitializeComponent();
        ...
        private void Btn_OK_Click(object sender, EventArgs e)
        {
            if (this.TB_Password.Text == "1234")
            {
                ...
                { 
                    ...
                    string SearchDate = " select w2.NextMaintDate ,  w1.MaintPeriodUnit, w1.MaintPeriord , w1.MaintDate  "
                                      + " from Jig w1 join JigInfo w2 on w1.jig = w2.jig   where w1.jig = '" + JigCode + "' ";
            ...
            {
                MessageBox.Show("Error! Password is fault.", "Error", MessageBoxIcon.Error);
                return;
            }
        }

        //private void JigRepairConfirm_Load(object sender, EventArgs e)
        //{
        //    Process();
        //}
        //public void Process()
        ...
        //    JigMaintHist = this.e.DataGridView.CurrentRow.Cells["JigMaintHist"].Value.ToString();
        //    JigCode = this.e.DataGridView.CurrentRow.Cells["Jig"].Value.ToString();
        //    if (string.IsNullOrEmpty(this.e.DataGridView.CurrentRow.Cells["JigMaintHist"].Value.ToString()))
        //    {
        //        MessageBox.Show("Error! is not confirm data.", "Error", MessageBoxIcon.Error);
