...
namespace WiseM.Browser
{
    public partial class RePackingNew : Form
    {
        ...
        {
            var typeList = new BindingList<object>
            {
                new {Key = "", Value = ""},
                new {Key = "Box", Value = "Box"},
                new {Key = "Pallet", Value = "Pallet"}
            };
            ...
            {
                new {Key = "", Value = ""},
                new {Key = "Packing", Value = "Đóng gói(Packing)"},
...
;
        ...
        private void button_Created_Click(object sender, EventArgs e)
        {
            ...
            {
                MessageBox.Show("Vui lòng chọn loại hình。(Please select the type.)", "Cảnh báo(Warning)", MessageBoxButtons.OK, MessageBoxIcon.Warning);
                return;
            }

            if (string.IsNullOrEmpty(comboBox_Location.SelectedValue.ToString()))
            {
                MessageBox.Show("Vui lòng chọn loại hình。(Please select the location.)", "Cảnh báo(Warning)", MessageBoxButtons.OK, MessageBoxIcon.Warning);
                return;
            }

            if (string.IsNullOrEmpty(comboBox_Line.SelectedValue.ToString()))
            {
                MessageBox.Show("请输入产品。(Please select the Line.)", "Cảnh báo(Warning)", MessageBoxButtons.OK, MessageBoxIcon.Warning);
                return;
            }

            if (string.IsNullOrEmpty(dateTimePicker_PackDate.Text))
            {
                MessageBox.Show("Vui lòng nhập vật liệu。(Please enter the date.)", "Cảnh báo(Warning)", MessageBoxButtons.OK, MessageBoxIcon.Warning);
                return;
            }

            if (string.IsNullOrEmpty(textBox_Material.Text))
            {
                MessageBox.Show("请输入产品。(Please select the Material.)", "Cảnh báo(Warning)", MessageBoxButtons.OK, MessageBoxIcon.Warning);
                return;
            }
        ...
        private void dataGridView_Material_CellDoubleClick(object sender, DataGridViewCellEventArgs e)
        {
            ...
            {
                MessageBox.Show("Vui lòng nhập thông tin kiểm tra sản phẩm。(Please enter the product reference information.) - LG_ITEM_CD", "Cảnh báo(Warning)", MessageBoxButtons.OK, MessageBoxIcon.Warning);
                return;
            }
            ...
            {
                MessageBox.Show("Vui lòng nhập thông tin kiểm tra sản phẩm。(Please enter the product reference information.) - LG_ITEM_NM", "Cảnh báo(Warning)", MessageBoxButtons.OK, MessageBoxIcon.Warning);
                return;
            }
