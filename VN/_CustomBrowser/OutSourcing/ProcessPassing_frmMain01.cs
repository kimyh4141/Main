...
namespace WiseM.Browser
{
    public partial class ProcessPassing_frmMain01 : Form
    {
        ...
        private void btnSave_Click(object sender, EventArgs e)
        {
            try
            {
                if (dtWO.Rows.Count < 1)
                    return;

                int intOrderQty = Convert.ToInt32(dtWO.Rows[0]["OrderQty"]);
                int intActualQty = Convert.ToInt32(dtWO.Rows[0]["ActualQty"]);
                ...
                if (intRC != 0)
                {
                    string strMsg = ds1.Tables[ds1.Tables.Count - 1].Rows[0]["ERR_MSG"].ToString();
                    MessageBox.Show(strMsg, "Error", MessageBoxButtons.OK, MessageBoxIcon.Error);
                    if (intRC != -999) InsertIntoSysLog(strMsg);
                    return;
                }
                ...
                /*
                     2023-09-22 gmryu - SP에서 모두 OutputHist에 Insert 하기로 해서 사용안함 

                     if (strActiveStatus == "Active")
                {
                    string strClientId = ds1.Tables[ds1.Tables.Count - 2].Rows[0]["ClientId"].ToString();
                    string strActiveJob = ds1.Tables[ds1.Tables.Count - 2].Rows[0]["ActiveJob"].ToString(); // ?????
                    string strWorkCenter = ds1.Tables[ds1.Tables.Count - 2].Rows[0]["Workcenter"].ToString();
                    ...
                    var r = new EntryRequest
                    {
                        ...
                        ActiveJob = strActiveJob,       // ??????????????????????????????
                        Workcenter = strWorkCenter,
                        Count = 1,
                        Cavity = 1
                    };
