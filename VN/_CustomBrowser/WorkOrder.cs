...
namespace WiseM.Browser
{
    public partial class WorkOrder : SkinForm
    {
        ...
        private void btn_Search_Click(object sender, EventArgs e)
        {
            ...
            // ERP 작업지시 조회
            query = $@"
                     SELECT EPPO.PRODT_ORDER_NO                                            AS WorkOrder
                      ...
                      FROM (
                           SELECT ROW_NUMBER() OVER (PARTITION BY EPPO.PRODT_ORDER_NO ORDER BY EPPO.IF_TIME DESC) AS RowNumber
                                ...
                                , EPPO.REMARK
	                            , EPPO.ORDER_STATUS
                             FROM MES_IF_VN.dbo.ETM_P_PROD_ORDER EPPO
                            WHERE 1 = 1
                              AND EPPO.PRODT_ORDER_NO LIKE '%{workOrder}%'
                           )                        AS                   EPPO
                           LEFT OUTER JOIN MES_IF_VN.dbo.M_P_WORK_CENTER MPWC
                       ...
                       AND isnull(EPPO.ORDER_STATUS ,'') != 'CL' 
            ...
            /*
             //gmryu 2023-09-04 사용안함
               //gmryu 2023-09-03 임시로 사용
            query = $@"
                    SELECT EPPO.PRODT_ORDER_NO                                             AS WorkOrder
                         , EPPO.ITEM_CD                                                    AS Material
                         , M.Spec                                                          AS Spec
                         ...
                         , COALESCE(EPPO.WC_CD, '')                                        AS WC_CD
                         , MPWC.WC_NM
                         , Z.MINOR_CD                                                      AS ROUT_SEQ
                         , Z.MINOR_NM                                                      AS ROUT_SEQ_NM
                         , CONVERT(INT, EPPO.ORDER_QTY)                                    AS [Order Qty]
                         , COALESCE(ConfirmQty, 0)                                         AS ConfirmQty
                         , IIF(( EPPO.ORDER_QTY - COALESCE(ConfirmQty, 0) ) <= 0, 'Y', '') AS Ordered
                         , CONVERT(VARCHAR(20), EPPO.PLAN_START_DT, 23)                    AS [Plan Start Date]
                      ...
                      FROM (
						...
						left outer join  [MES_IF_VN].[dbo].[M_B_MINOR_ZP001]  AS        Z
						   on M.ROUT_SEQ = [MINOR_CD]       
            ...
            this.dgv_Search.Columns["REMARK"].Width = 80;
        ...
        private void dgv_Search_CellDoubleClick(object sender, DataGridViewCellEventArgs e)
        {
            ...
            //2023-11-03 gmryu 1팔레트 PCB 수량 계산 추가
            if (lbl_routing.Text == "Pk_StockIn" || lbl_routing.Text == "Pk_Boxing")
            {
                lbl_PalletQty.Text = "";
                Num_UpDown_PalletQty.Value = 1;
                lbl_PalletQty.Visible = true;
                Num_UpDown_PalletQty.Visible = true;
                Pallet_Per_PCB();
            ...
            else
            {
                lbl_PalletQty.Text = "";
                Num_UpDown_PalletQty.Value = 1;
                lbl_PalletQty.Visible = false;
                Num_UpDown_PalletQty.Visible = false;
            }
        ...
        private void btn_Add_WorkCenter_Click(object sender, EventArgs e)
        {
            ...
            {
                System.Windows.Forms.MessageBox.Show("Vượt quá số lượng！\r\n" + "Quantity Over!", "Lỗi(Fail)", MessageBoxButtons.OK, MessageBoxIcon.Stop);
                this.num_Qty.Focus();
                return;
            }

            //gmryu 2023-11-03 입고 지시 팔레트 수량과 딱 맞아 떨어지지 않을경우 경고 출력 
            Boolean Gridview_Red_Color = false;
            if (lbl_routing.Text == "Pk_StockIn" || lbl_routing.Text == "Pk_Boxing")
            {
                try
                {

                    string _material = this.dgv_Search.Rows.OfType<DataGridViewRow>().Where(r => r.Cells["WorkOrder"].Value.Equals(this.lbl_SelectedWorkOrder.Text)).Select(r => Convert.ToString(r.Cells["Material"].Value)).FirstOrDefault();

                    decimal tempQty = 0;
                    tempQty = Convert.ToDecimal(DbAccess.Default.ExecuteScalar(
                        $@" select ((cast(QtyInBox  as decimal) * BoxQtyInPallet)) as CountPalletPerPCB 
                            from Material 
                            where Material = '{_material}' "));

                    //System.Windows.Forms.MessageBox.Show(remainQty+"remain "+num_Qty.Value+"inputQty "+tempQty +"PalletQty "+ remainQty % (tempQty) + "Result");
                    if (num_Qty.Value % tempQty != 0)
                    {
                        if (System.Windows.Forms.MessageBox.Show("PalletQty and InputQty Didn't Match!\r\nAre you Sure?", "Chú ý (Notice)", MessageBoxButtons.YesNo, MessageBoxIcon.Question) == DialogResult.No)
                            return;

                        Gridview_Red_Color = true;
                    }
                    else
                    {
                        Gridview_Red_Color = false;
                    }
                    // lbl_totalQty.Text = (remainQty - num_Qty.Value) + "";
                ...
                catch (Exception)
                {
                    ...
                    System.Windows.Forms.MessageBox.Show("BoxQty or PalletQty Check!");
                }
            }
            ...
            if (Gridview_Red_Color == true)
            {
                this.dgv_Confirm.Rows[rowIndex].Cells["colQty"].Style.ForeColor = System.Drawing.Color.Red;
                Gridview_Red_Color = false;
        ...
        private void btn_Confirm_Click(object sender, EventArgs e)
        {
            ...
            if (dgv_Confirm.Rows.Count > 0)
            {
                  ...
                  try
                {
                    ...
                    AND EPPO.I_PROC_STEP IN('C', 'U')                 
                    AND EPPO.PRODT_ORDER_NO = '{lbl_SelectedWorkOrder.Text}' ";

                    string WorkOrderQuery = $" select sum(OrderQty) from WorkOrder where ERP_WorkOrder = '{lbl_SelectedWorkOrder.Text}' and  Routing = '{lbl_routing.Text}'  ";
                    double OrderQty = Convert.ToDouble(DbAccess.Default.ExecuteScalar(OrderQtyQuery)); //ERP 작업지시 수량
                    double WorkOrderQty = Convert.ToDouble(DbAccess.Default.ExecuteScalar(WorkOrderQuery)); //만들어진 MES 작업지시 수량
                    double CompareQty = 0; //만들어질 MES 작업지시 수량
                    foreach (DataGridViewRow row in dgv_Confirm.Rows)
                    {
                        if (Convert.ToBoolean(row.Cells["colConfirmed"].Value).Equals(false) && Convert.ToBoolean(row.Cells["colCheck"].Value).Equals(true))
                        {
                            CompareQty += Convert.ToDouble(row.Cells["colQty"].Value);
                        }
                    }
                    ...
                    if (string.IsNullOrEmpty(CompareQty.ToString()))
                    {
                        CompareQty = 0;
                    }
                    if (string.IsNullOrEmpty(WorkOrderQty.ToString()))
                    {
                        WorkOrderQty = 0;
                    }

                    if (OrderQty < (CompareQty + WorkOrderQty))
                    {
                        System.Windows.Forms.MessageBox.Show("ERP Order QTY : " + OrderQty + Environment.NewLine + "WorkOrder QTY : " + (CompareQty + WorkOrderQty), "Search WorkOrder Again", MessageBoxButtons.OK, MessageBoxIcon.Error);
                        return;
                    }    
                }
                catch (Exception ex)
                ...
                query.AppendLine
                    (
                     $@"
                    UPDATE MES_IF_VN.dbo.ETM_P_PROD_ORDER
                       SET APPLY_FLAG = 'Y'
                         , APPLY_TIME = GETDATE()
                ...
                try
                {
                   // System.Windows.Forms.MessageBox.Show(query.ToString());
                    ...
                    BindingSelectOrder(dgv_Search.CurrentCell.OwningRow); //2023-10-24 ERP 작업지시 초과 방지
                    System.Windows.Forms.MessageBox.Show("Lưu xong。\r\nSave Complete.", "Notice", MessageBoxButtons.OK, MessageBoxIcon.Exclamation);
                }
                catch (Exception ex)
                {
        ...
        private void btn_Del_Click(object sender, EventArgs e)
        {
            ...
            if (count <= 0)
            {
                MessageBox.ShowCaption("Không có dòng nào để xóa。\r\nThere are no lines to delete.", "Chú ý (Notice)", MessageBoxIcon.Error);
                return;
            }
            ...
            MessageBox.ShowCaption("Đã xóa。\r\nDelete Complete.", "Chú ý (Notice)", MessageBoxIcon.Asterisk);
        }
        ...
        private void btn_Insert_Click(object sender, EventArgs e)
        {
            ...
            if (System.Windows.Forms.MessageBox.Show("Bạn có chắc chắn không？\r\nAre you Sure?", "Chú ý (Notice)", MessageBoxButtons.YesNo, MessageBoxIcon.Question)
                == DialogResult.Yes)
            {
                try
                {
        ...
        private void Num_UpDown_PalletQty_ValueChanged(object sender, EventArgs e)
        {   //2023-11-03 gmryu 작업지시 수량이 팔레트 PCB 수량과 맞아 떨어지는지 확인 
            Pallet_Per_PCB();
        }
        private void Pallet_Per_PCB()
        {   
            //2023-11-03 gmryu 작업지시 수량이 팔레트 PCB 수량과 맞아 떨어지는지 확인 
            if (lbl_routing.Text == "Pk_StockIn" )
            {
                lbl_PalletQty.Text = "";
                string _material = this.dgv_Search.Rows.OfType<DataGridViewRow>().Where(r => r.Cells["WorkOrder"].Value.Equals(this.lbl_SelectedWorkOrder.Text)).Select(r => Convert.ToString(r.Cells["Material"].Value)).FirstOrDefault();
                ...
                lbl_PalletQty.Text = " Pallet : " + (Num_UpDown_PalletQty.Value * tempQty).ToString();
            }
            else if (lbl_routing.Text == "Pk_Boxing")
            {
                lbl_PalletQty.Text = "";
                string _material = this.dgv_Search.Rows.OfType<DataGridViewRow>().Where(r => r.Cells["WorkOrder"].Value.Equals(this.lbl_SelectedWorkOrder.Text)).Select(r => Convert.ToString(r.Cells["Material"].Value)).FirstOrDefault();
                ...
                tempQty = Convert.ToDecimal(DbAccess.Default.ExecuteScalar(
                    $@" select ((cast(QtyInBox  as decimal) * BoxQtyInPallet)) as CountPalletPerPCB 
                            from Material 
                            where Material = (	select TOP_ITEM_CD from Material    where Material = '{_material}') "));
